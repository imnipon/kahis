...
เอกสารนี้ระบุตารางฟิลด์ข้อมูลทั้งหมด **51 ฟิลด์** (แบ่งเป็น 9 ตารางย่อย ครอบคลุม 8 เซกชัน และข้อมูลผู้ใช้) สำหรับระบบ Vital & PE Editor โดยอ้างอิงตามแนวทางมาตรฐานขององค์กรสัตวแพทย์สากล เช่น AAHA, ACVIM, WSAVA, RECOVER, Merck Veterinary Manual และ UC Davis School of Veterinary Medicine
...
|---|-------------|--------|---------------------|--------------------------|
| 1 | Temp | `text` (numeric inputmode) | Field hint: `100.0–102.5 °F` (Canine) / `100.5–102.5 °F` (Feline) | Merck Vet Manual / AAHA Vital Signs Guidelines |
| 2 | HR | `text` (numeric inputmode) | Field hint: `60–140 bpm` (Canine) / `140–220 bpm` (Feline) | UC Davis Small Animal Practice / VIN |
| 3 | RR | `text` (numeric inputmode) | Field hint: `15–30 rpm` (Canine) / `20–30 rpm` (Feline) | ACVIM Pulmonology / Merck Vet Manual |
| 4 | Pulse | `text` (numeric inputmode) | Field hint: `60–140 bpm` (Canine) / `140–220 bpm` (Feline) | Matches Heart Rate / Core Physical Assessment |
| 5 | Weight | `text` (numeric inputmode) | Field hint: `10.00 kg` (Canine) / `4.50 kg` (Feline) | Clinical Practice Baseline |
...
| 7 | FBS | `text` (numeric inputmode) | Field hint: `75–120 mg/dL` (Canine) / `70–150 mg/dL` (Feline) — ความเครียดทำให้แมวน้ำตาลขึ้นสูงชั่วคราวได้ง่าย | Merck Vet Manual / Plumb's Veterinary Drugs |
...
| 9 | Estimated Dehydration | `text` (numeric inputmode) | Field hint: `0%` (ช่วงที่กรอกได้คือ 0–12%) | AAHA Fluid Therapy Guidelines |
...
| # | ฟิลด์ข้อมูล | ประเภท | Field Hint / Placeholder / Options | มาตรฐานสากลที่ใช้อ้างอิง |
|---|-------------|--------|---------------------|--------------------------|
| 10 | NIBP SYS | `text` (numeric inputmode) | Field hint: `110–140 mmHg` (Canine) / `120–140 mmHg` (Feline) | ACVIM Consensus Statement (Hypertension) |
| 11 | NIBP DIA | `text` (numeric inputmode) | Field hint: `60–90 mmHg` (Canine) / `70–90 mmHg` (Feline) | ACVIM Consensus Statement (Hypertension) |
| 12 | NIBP MAP | `text` (numeric inputmode) | Field hint: `70–110 mmHg` (Canine) / `80–110 mmHg` (Feline) | ACVIM Consensus Statement (Hypertension) |
...
| 15 | SpO2 | `text` (numeric inputmode) | Field hint: `95–100%` | VECCS Critical Care / Anesthesia Monitoring |
| 16 | EtCO2 | `text` (numeric inputmode) | Field hint: `35–45 mmHg` | ACVIM Consensus / ACVA Anesthesia Guidelines |
...
| 24 | Critical Event Time | `datetime-local` | Example format: `YYYY-MM-DD hh:mm` | Clinical Documentation System Standard |
...
| 47 | Observed Pain Indicators | `checkbox` | Options: `Vocalization`, `Facial Expression (Grimace)`, `Abnormal Posture / Guarding`, `Restlessness / Agitation`, `Reduced Movement / Lethargy`, `Aggression / Resistance to touch`, `Anorexia` | WSAVA / Feline Grimace Scale Guidelines |
